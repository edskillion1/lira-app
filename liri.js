...
function getTweets() {
	...
	const queryUrl = "https://api.twitter.com/1.1/search/tweets.json?q=edskillion&result_type=recent&count=20";

	client.get(queryUrl, (error, tweets, response) => {

		if (error) {
			logErrors('getTweets()', '@edskillion');
			console.log(error);
		}

		console.log("\nHere are the latest tweets from @edskillion: \n".cyan);
	...
	logStuffThatHappens('my-tweets           ', '@edskillion');
...
function doWhatItSays() {
	fs.readFile("random.txt", "utf8", function(error, data){
		if (!error) {
			...
			getMusic(doWhatItSaysResults[0], doWhatItSaysResults[1]);
		} else {
